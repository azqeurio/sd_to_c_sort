...
For convenience, pre-built Windows executables are provided as a zip archive.

1. Download the latest release zip from GitHub Releases.
2. Extract the zip file (`sd_to_c_sort_windows.zip`).
3. Run `sd_to_c_sort.exe`.
...
편의상 미리 빌드된 Windows 실행 파일을 zip 압축본으로 제공합니다.

1. 깃허브 Release 페이지에서 최신 zip 파일(`sd_to_c_sort_windows.zip`) 다운로드.
2. 압축 해제.
3. `sd_to_c_sort.exe` 실행.
